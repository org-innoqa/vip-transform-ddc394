...
function App() {
  ...
  useEffect(() => { document.documentElement.lang = language; document.documentElement.dir = language === 'ar' ? 'rtl' : 'ltr'; window.scrollTo({ top: 0, behavior: 'smooth' }) }, [path, language])

  const navigate = (to: string) => { window.history.pushState({}, '', to); setPath(to); setMenuOpen(false) }
  const isHome = path === '/'
...
function FleetGrid({ navigate }: { navigate: (to: string) => void }) { return <div className="fleet-grid">{fleet.map((car) => <article className="fleet-card" key={car.name}><img src={car.image} alt={`${car.name} lüks araç`} loading="lazy" /><div className="fleet-info"><div><h3>{car.name}</h3><p>{car.meta}</p><small>{car.equipment}</small></div><button onClick={() => navigate('/rezervasyon')} aria-label={`${car.name} ile teklif al`}><ArrowRight size={20} /></button></div></article>)}</div> }
...
function ReservationSteps() { return <div className="steps-grid">{['Güzergâh', 'Araç seçimi', 'Ekstralar', 'İletişim ve onay'].map((step, i) => <article className="step-card" key={step}><span>0{i + 1}</span><Check size={18} /><h3>{step}</h3><p>{['Hizmet tipi, bölgeler, tarih-saat ve yolcu bilgileri.', 'Kapasitenize uygun araçları fiyatlarıyla görün.', 'Çocuk koltuğu, karşılama tabelası ve notunuzu ekleyin.', 'Bilgilerinizi ve KVKK onayınızı tamamlayın.'][i]}</p></article>)}</div> }
...
function Contact() { return <div className="contact-layout"><div className="contact-note"><h2>Doğru plan, <em>iyi başlangıç.</em></h2><p>Telefon, WhatsApp ve e-posta bilgileriniz hazır olduğunda bu alan üzerinden müşterilerinizle doğrudan buluşacak.</p></div><form className="contact-form" onSubmit={(e) => e.preventDefault()}><label>Ad soyad<input required name="name" placeholder="Adınız ve soyadınız" /></label><label>E-posta<input required type="email" name="email" placeholder="ornek@eposta.com" /></label><label>Mesajınız<textarea required name="message" rows={5} placeholder="Transfer planınızı kısaca anlatın" /></label><button className="button button-primary" type="submit">Mesaj gönder <ArrowRight size={17} /></button></form></div> }
