...
type PricingSettings = { id: number; round_trip_discount_percent: number; night_surcharge_percent: number; currency_code: string; operations_email?: string | null }
...
export default function AdminPanel() {
  ...
  const [pricingSettings, setPricingSettings] = useState<PricingSettings>({ id: 1, round_trip_discount_percent: 0, night_surcharge_percent: 0, currency_code: 'QAR', operations_email: '' })
  ...
  async function loadData() {
    ...
    try {
      const [r, v, z, e, pr, s] = await Promise.all([
        ...
        db.select<PricingSettings>('pricing_settings', { limit: 1 }),
      ])
      setReservations(r); setVehicles(v); setZones(z); setExtras(e); setPriceRules(pr); setPricingSettings(s[0] || pricingSettings)
  ...
  async function savePricingSettings(e: FormEvent) {
    e.preventDefault(); setError('')
    const operations_email = (pricingSettings.operations_email || '').trim().toLowerCase()
    if (operations_email && !/^\S+@\S+\.\S+$/.test(operations_email)) return void fail('Şirket bildirim e-postası geçerli olmalıdır.')
    try { await db.update('pricing_settings', '?id=eq.1', { operations_email: operations_email || null }); setPricingSettings({ ...pricingSettings, operations_email }); setError('Bildirim e-postası kaydedildi.') } catch (err) { setError(friendlyError(err, 'Bildirim ayarı kaydedilemedi.')) }
  }
  ...
  const tabs = [['reservations', 'Rezervasyonlar'], ['vehicles', 'Araçlar'], ['zones', 'Bölgeler'], ['priceRules', 'Tarife tablosu'], ['extras', 'Ekstralar'], ['settings', 'Bildirim ayarı']]
  return <section className="admin-shell section-wrap"><div className="admin-head"><div><p className="eyebrow">OPERASYON PANELİ</p><h1>Hoş geldiniz, {username}</h1></div><div className="admin-actions"><button className="button button-outline" onClick={() => void loadData()} disabled={loading}><RefreshCw size={16} /> Yenile</button><button className="button button-outline" onClick={() => void logout()}><LogOut size={16} /> Çıkış</button></div></div>{error && <p className="form-error" role="alert">{error}</p>}<nav className="admin-tabs" aria-label="Yönetim bölümleri">{tabs.map(([id, label]) => <button key={id} className={tab === id ? 'active' : ''} onClick={() => setTab(id)}>{label}</button>)}</nav>{loading && <p className="admin-loading">Veriler yükleniyor…</p>}
    ...
    {tab === 'settings' && <div className="admin-grid"><form className="admin-form" onSubmit={savePricingSettings}><h2>Bildirim ayarı</h2><p>Yeni rezervasyon bildirimleri bu adrese kuyruklanır. Müşteri e-postası ayrıca otomatik oluşturulur.</p><label>Şirket bildirim e-postası<input type="email" value={pricingSettings.operations_email || ''} onChange={e => setPricingSettings({ ...pricingSettings, operations_email: e.target.value })} placeholder="operasyon@alanadiniz.com" /></label><button className="button button-primary"><Save size={16} /> Kaydet</button></form></div>}
  </section>
}
