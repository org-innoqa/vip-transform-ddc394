...
type Reservation = { id: string; code: string; service_type: string; customer_name: string; customer_phone: string; customer_email: string; calculated_amount: number; status: string; pickup_at: string; passenger_count: number; luggage_count: number; flight_number?: string | null; customer_note?: string | null }
...
export default function AdminPanel() {
  ...
  async function saveVehicle(e: FormEvent) {
    ...
    if (!value.model || value.passenger_capacity < 1 || value.luggage_capacity < 0 || value.multiplier <= 0) return void fail('Model, kapasite, bagaj ve pozitif çarpan zorunludur.')
  ...
  async function saveZone(e: FormEvent) {
    ...
    try { editingZone ? await db.update('zones', `?id=eq.${editingZone}`, { name }) : await db.insert('zones', { name, is_active: true }); setZoneForm({ name: '' }); setEditingZone(null); await loadData() } catch (err) { setError(err instanceof Error ? err.message : 'Bölge kaydedilemedi.') }
  }
  async function savePriceRule(e: FormEvent) {
    e.preventDefault(); const base_price = Number(priceForm.base_price)
    if (!priceForm.origin_zone_id || !priceForm.destination_zone_id || priceForm.origin_zone_id === priceForm.destination_zone_id || !Number.isFinite(base_price) || base_price < 0) return void fail('Farklı kalkış-varış bölgeleri ve geçerli fiyat zorunludur.')
    try { editingPriceRule ? await db.update('price_rules', `?id=eq.${editingPriceRule}`, { ...priceForm, base_price }) : await db.insert('price_rules', { ...priceForm, base_price, is_active: true }); setPriceForm({ origin_zone_id: '', destination_zone_id: '', base_price: 0 }); setEditingPriceRule(null); await loadData() } catch (err) { setError(err instanceof Error ? err.message : 'Tarife kaydedilemedi.') }
  }
  async function saveExtra(e: FormEvent) {
    e.preventDefault(); const name = extraForm.name.trim(); const price = Number(extraForm.price)
    if (!name || !Number.isFinite(price) || price < 0) return void fail('Ekstra adı ve sıfırdan büyük/eşit fiyat zorunludur.')
    try { editingExtra ? await db.update('extras', `?id=eq.${editingExtra}`, { ...extraForm, name, price }) : await db.insert('extras', { ...extraForm, name, price, is_active: true }); setExtraForm(emptyExtra); setEditingExtra(null); await loadData() } catch (err) { setError(err instanceof Error ? err.message : 'Ekstra kaydedilemedi.') }
  }
  async function updateStatus(id: string, status: string) { try { await db.update('reservations', `?id=eq.${id}`, { status }); setReservations(items => items.map(r => r.id === id ? { ...r, status } : r)); setSelectedReservation(item => item?.id === id ? { ...item, status } : item) } catch (err) { setError(err instanceof Error ? err.message : 'Durum güncellenemedi.') } }
